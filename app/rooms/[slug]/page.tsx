...
export async function generateMetadata({ params }: { params: Promise<RoomPageParams> }): Promise<Metadata> {
  const { slug } = await params;
  const room = await getRoom(slug);
  if (!room) return buildMetadata({ title: "Room Not Found", description: "This room could not be found.", path: `/rooms/${slug}` });
  return buildMetadata({ title: room.title, description: room.description, path: `/rooms/${room.slug}`, image: room.heroImage.src });
}

export default async function RoomDetailPage({ params }: { params: Promise<RoomPageParams> }) {
  const { slug } = await params;
  const room = await getRoom(slug);
  if (!room) notFound();
  ...
  return (
    <main>
      <Navbar />
      <PageHero eyebrow={room.subtitle} title={room.title} subtitle={`Starting From ₹${room.priceFrom.toLocaleString("en-IN")} / Night`} breadcrumbItems={[{ label: "Home", href: "/" }, { label: "Rooms", href: "/rooms" }, { label: room.title }]} backgroundImage={room.heroImage.src} />
      <div className="flex justify-center bg-[#0F1720] pb-16 sm:pb-20">
        <Link href={`/book-now?room=${encodeURIComponent(room.slug)}`} className="inline-flex items-center justify-center rounded-sm bg-[#C9A24A] px-9 py-4 font-body text-xs font-medium uppercase tracking-[0.25em] text-[#0F1720] transition-all duration-300 hover:-translate-y-0.5 hover:bg-[#d9b563] hover:shadow-[0_15px_45px_-10px_rgba(201,162,74,0.55)] sm:text-sm">Book Now</Link>
      </div>
          ...
          <div className="mt-16 grid grid-cols-1 gap-14 lg:mt-20 lg:grid-cols-[62%_38%] lg:gap-16">
            <div>
              ...
              <ul className="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-3">
                {room.amenities.map((amenity) => {
                  ...
                  return <li key={amenity} className="flex items-center gap-2.5 rounded-xl border border-[#0F1720]/8 bg-white/70 px-4 py-3"><Icon size={16} strokeWidth={1.5} className="shrink-0 text-[#C9A24A]" /><span className="font-body text-xs text-[#0F1720]/75 sm:text-sm">{amenity}</span></li>;
                })}
              </ul>
              ...
              <div className="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-3">
                ...
                ].map(({ icon: Icon, label, value }) => <div key={label} className="rounded-2xl border border-[#0F1720]/8 bg-white/70 p-5 text-center shadow-[0_12px_35px_-20px_rgba(15,23,32,0.2)]"><Icon size={20} strokeWidth={1.5} className="mx-auto text-[#C9A24A]" /><p className="mt-3 font-body text-xs uppercase tracking-[0.12em] text-[#0F1720]/50">{label}</p><p className="mt-1 font-display text-sm font-medium text-[#0F1720]">{value}</p></div>)}
              </div>
            </div>
            ...
            <div className="lg:sticky lg:top-32 lg:h-fit">
              <div className="rounded-[28px] border border-[#C9A24A]/25 bg-[#111827] p-7 shadow-[0_30px_80px_-25px_rgba(0,0,0,0.5)] sm:p-8">
                ...
                <p className="mt-2 font-display text-3xl font-light text-[#F8F8F5]">₹{room.priceFrom.toLocaleString("en-IN")}<span className="font-body text-sm font-light text-[#F8F8F5]/50"> / Night</span></p>
      ...
      <section className="bg-[#0E141B] px-6 py-24 sm:px-10 sm:py-28">
        <div className="mx-auto max-w-7xl">
          <div className="mx-auto max-w-2xl text-center"><p className="font-body text-xs font-medium uppercase tracking-[0.4em] text-[#C9A24A]">MORE OPTIONS</p><h2 className="mt-5 font-display text-3xl font-light leading-tight text-[#F8F8F5] sm:text-4xl">Similar Rooms</h2></div>
          <div className="mt-14 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:mt-16 lg:grid-cols-3">{similarRooms.map((similarRoom) => <RoomCard key={similarRoom.id} room={similarRoom} />)}</div>
        </div>
      </section>

      <section className="bg-[#F8F6F2] px-6 py-24 sm:px-10 sm:py-28">
        <div className="mx-auto max-w-3xl">
          <div className="text-center"><p className="font-body text-xs font-medium uppercase tracking-[0.4em] text-[#C9A24A]">{roomsPageContent.faqs.eyebrow}</p><h2 className="mt-5 font-display text-3xl font-light leading-tight text-[#0F1720] sm:text-4xl">{roomsPageContent.faqs.heading}</h2><div aria-hidden="true" className="mx-auto my-7 h-px w-20 bg-[#C9A24A]" /></div>
