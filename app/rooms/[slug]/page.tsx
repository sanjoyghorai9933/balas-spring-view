...
function getRoom(slug: string): Room | undefined {
  return roomsContent.rooms.find((room) => room.slug === slug);
}

export function generateStaticParams(): RoomPageParams[] {
  return roomsContent.rooms.map((room) => ({ slug: room.slug }));
}
...
export default async function RoomDetailPage({
  ...
  const similarRooms = roomsContent.rooms.filter((r) => r.id !== room.id);
    ...
    <main>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-16 grid grid-cols-1 gap-14 lg:mt-20 lg:grid-cols-[62%_38%] lg:gap-16">
            <div>
              ...
              <div
                aria-hidden="true"
                className="my-6 h-px w-16 bg-[#C9A24A]"
              />
              <p className="font-body text-sm font-light leading-relaxed text-[#0F1720]/65 sm:text-base">
                {room.longDescription}
              </p>

              <h3 className="mt-10 font-display text-xl font-medium text-[#0F1720]">
                Amenities
              </h3>
              <ul className="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-3">
                {room.amenities.map((amenity) => {
                  ...
                  return (
                    <li
                      key={amenity}
                      className="flex items-center gap-2.5 rounded-xl border border-[#0F1720]/8 bg-white/70 px-4 py-3"
                    >
                      <Icon size={16} strokeWidth={1.5} className="shrink-0 text-[#C9A24A]" />
                      <span className="font-body text-xs text-[#0F1720]/75 sm:text-sm">
                        {amenity}
                      </span>
                    </li>
                  );
              ...
              {/* 4. Room Information Cards */}
              <h3 className="mt-12 font-display text-xl font-medium text-[#0F1720]">
                Room Information
              </h3>
              <div className="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-3">
                {[
                  ...
                  <div
                    key={label}
                    className="rounded-2xl border border-[#0F1720]/8 bg-white/70 p-5 text-center shadow-[0_12px_35px_-20px_rgba(15,23,32,0.2)]"
                  >
                    <Icon size={20} strokeWidth={1.5} className="mx-auto text-[#C9A24A]" />
                    <p className="mt-3 font-body text-xs uppercase tracking-[0.12em] text-[#0F1720]/50">
                      {label}
                    </p>
            ...
            {/* 5. Booking Card (sticky on desktop) */}
            <div className="lg:sticky lg:top-32 lg:h-fit">
              <div className="rounded-[28px] border border-[#C9A24A]/25 bg-[#111827] p-7 shadow-[0_30px_80px_-25px_rgba(0,0,0,0.5)] sm:p-8">
                ...
                <p className="mt-2 font-display text-3xl font-light text-[#F8F8F5]">
                  ...
                  <span className="font-body text-sm font-light text-[#F8F8F5]/50">
                    {" "}
                    / Night
                  </span>
                </p>
      ...
      <section className="bg-[#F8F6F2] px-6 py-24 sm:px-10 sm:py-28">
        <div className="mx-auto max-w-3xl">
          ...
          <div className="mt-12 text-[#0F1720]">
            <Accordion items={roomsPageContent.faqs.items} />
          </div>
        </div>
      </section>
