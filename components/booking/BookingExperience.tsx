...
export default function BookingExperience() {
  ...
  const [rooms, setRooms] = useState<Room[]>(roomsContent.rooms);
  ...
  useEffect(() => {
    let cancelled = false;
    void fetch("/api/content/rooms", { cache: "no-store" })
      .then((response) => response.ok ? response.json() : null)
      .then((content: { rooms?: Room[] } | null) => {
        if (cancelled || !content?.rooms?.length) return;
        setRooms(content.rooms);
        setRoomId((current) => content.rooms?.some((room) => room.id === current || room.slug === current) ? current : content.rooms?.[0]?.id ?? "");
      })
      .catch(() => undefined);
  ...
  useEffect(() => {
    ...
    if (!roomParam) return;
    const room = rooms.find((item) => item.id === roomParam || item.slug === roomParam);
    if (room) setRoomId(room.id);
  }, [rooms, searchParams]);

  const selectedRoom = useMemo(() => rooms.find((room) => room.id === roomId) ?? rooms[0], [rooms, roomId]);
        ...
        <div className="grid grid-cols-1 gap-14 lg:grid-cols-[40%_60%] lg:gap-12">
          <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.7 }} className="space-y-6">
            <div className="rounded-[28px] border border-[#C9A24A]/25 bg-[#111827]/60 p-7 shadow-[0_25px_65px_-25px_rgba(0,0,0,0.5)] sm:p-8">
              ...
              <div className="mt-5 space-y-3 border-t border-[#F8F8F5]/10 pt-5"><div className="flex items-start gap-2.5 text-[#B9B9B9]"><MapPin size={15} className="mt-0.5 shrink-0 text-[#C9A24A]" /><span className="font-body text-sm">Mussoorie, Uttarakhand</span></div><div className="flex items-start gap-2.5 text-[#B9B9B9]"><Clock size={15} className="mt-0.5 shrink-0 text-[#C9A24A]" /><span className="font-body text-sm">Check-in: {siteConfig.checkIn} &nbsp;•&nbsp; Check-out: {siteConfig.checkOut}</span></div></div>
